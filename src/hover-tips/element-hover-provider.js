const { HoverProvider, TextDocument, Position, Range, workspace, MarkdownString, Hover } = require('vscode');
const { ElDocument, localDocument } = require('../document');
...
class ElementHoverProvier {
  ...
  getHoverInstance(tag, attr, range) {
    const config = workspace.getConfiguration().get('elementv-snippet');
   ...
   */
  createHoverInstance(language, tag, attr, range) {
    let document = localDocument[language];
    if (tag === attr) {
      attr = '';
    }
    if (Object.prototype.hasOwnProperty.call(document, tag)) {
      ...
      Object.keys(tagDocument).forEach((key) => {
        const hoverMarkdownString = HoverDocumentGenerator.getInstance().generate(tagDocument, key, tag, attr, language);
